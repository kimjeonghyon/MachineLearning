...
for in_file in files:
    if in_file.endswith("sorted"):
        lst = open(in_file, 'r')
        lines = lst.readlines()[1:]
        lst.close()
        new_lines = ["ID,NamePoint,Line,Bus,Hour,DayMonth,Month,Year,Velocity,LatitudePoint,LongitudePoint,TimeStamp,DescriptionPoint,Speed\n"]
        dic_data = {}
        for row in lines:
            speed = 0.0
            r = row.split(",")
        ...
        for k in dic_data.keys():
            ...
            for ind, l in enumerate(csv_lines):
                if (ind == len(csv_lines)-1): break
                l0 = csv_lines[ind].split(",")
                l1 = csv_lines[ind + 1].split(",")
                lat0, lon0, tmstp0, lat1, lon1, tmstp1 = \
                l0[9], l0[10], \
                time.mktime(dateutil.parser.parse(l0[11][:10]+' '+l0[11][10:]).timetuple()), \
                l1[9], l1[10], \
                time.mktime(dateutil.parser.parse(l1[11][:10]+' ' +l1[11][10:]).timetuple())
                if (tmstp0 < tmstp1) :
                    speed = compute_speed(float(lat0), float(lon0), float(tmstp0), \
                                        float(lat1), float(lon1), float(tmstp1))
