...
def create_hdf5(in_dir, out_dir):
    ...
    lat_m, long_m = 36.3504761,127.3824999 #center of RJ coords
    day_m = 3600.*12. #mid_day in seconds
    for fp in files:
        data = list(csv.DictReader(open(fp, 'r'), delimiter=';'))
        if data[0]['ROT_NO'] in lines_dic: #only goes if the line is in the definition
            data_size = len(data)
            if len(range(time_size,data_size,stride))>0:
                ...
                for j in range(time_size,data_size,stride):
                    ...
                    for k in range(time_size):
                        item = data[j-k]
                        timestamp = dateutil.parser.parse(item['PASS_DTIME']) #the start date starts at 0 minutes of the hour
                        data4D[i,0,0,k] = (float(item['LATTD'])-lat_m)/0.1#/0.5 #divide by 0.1 in order to get larger numbers
                        data4D[i,1,0,k] = (float(item['LONGTD'])-long_m)/0.1#/0.5  #divide by 0.1 in order to get larger numbers
                    ...
                    label2D[i,0] = lines_dic[item['ROT_NO']] #Get only the last ground_truth
            ...
            print ('line not found:',data[0]['ROT_NO'])
            pass
